...
export class BlockContext implements PartialParse {
  ...
  advance() {
    ...
    for (;;) {
      for (let markI = 0; ; ) {
        let next =
          line.depth < this.stack.length
            ? this.stack[this.stack.length - 1]
            : null;
        while (
          markI < line.markers.length &&
          (!next || line.markers[markI].from < next.end)
        ) {
...
const EmphasisUnderscore: DelimiterType = {
  resolve: "Emphasis",
  mark: "EmphasisMark",
};
const EmphasisAsterisk: DelimiterType = {
  resolve: "Emphasis",
  mark: "EmphasisMark",
};
  ...
  LinkEnd(cx, next, start) {
    ...
    for (let i = cx.parts.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        let link = (cx.parts[i] = finishLink(
          cx,
          content,
  ...
  addDelimiter(
    ...
    return this.append(
      new InlineDelimiter(
        ...
        from,
        to,
        (open ? Mark.Open : Mark.None) | (close ? Mark.Close : Mark.None),
      ),
    );
  }
  ...
  takeNodes(cx: BlockContext) {
    ...
    for (;;) {
      ...
      if (cur.to - off <= cx.ranges[cx.rangeI].to) {
        ...
        let dummy = new Tree(
          cx.parser.nodeSet.types[Type.Paragraph],
          [],
          [],
          0,
          cx.block.hashProp,
