...
function initVim(CM) {
  ...
  function maybeInitVimState(cm) {
    if (!cm.state.vim) {
      ...
      cm.state.vim = {
        ...
        visualLine: false,
        visualBlock: false,
        lastSelection: /**@type{vimState["lastSelection"]}*/ (
          /**@type{unknown}*/ (null)
        ),
  ...
  */
    ...
    findKey: function (cm_, key, origin) {
          ...
          if (keysAreChars) {
            ...
            for (var i = 0; i < selections.length; i++) {
              ...
              var text = cm.getRange(
                from,
                cm.state.overwrite ? offsetCursor(to, 0, 1) : to,
              );
      ...
      if (command === false) {
        return !vim.insertMode &&
          (key.length === 1 || (CM.isMac && /<A-.>/.test(key)))
          ? function () {
              return true;
            }
          : undefined;
  ...
  "Left|Right|Up|Down|End|Home"
    .split("|")
    .concat(Object.keys(specialKey))
    .forEach(function (x) {
      vimToCmKeyMap[(specialKey[x] || "").toLowerCase()] = vimToCmKeyMap[
        x.toLowerCase()
      ] = x;
    });
  ...
  class RegisterController {
    ...
    isValidRegister(name) {
      return (
        name && (inArray(name, validRegisters) || latinCharRegex.test(name))
      );
    }
  ...
  var commandDispatcher = {
      ...
      if (!bestMatch) {
        if (matches.partial.length) {
          return {
            type: "partial",
            expectLiteralNext:
              matches.partial.length == 1 &&
              matches.partial[0].keys.slice(-11) == "<character>", // langmap literal logic
          };
        }
        return { type: "none" };
      }
      if (
        bestMatch.keys.slice(-11) == "<character>" ||
        bestMatch.keys.slice(-10) == "<register>"
      ) {
    ...
    processMotion: function (cm, vim, command) {
      ...
      vim.inputState.motionArgs = /**@type {MotionArgs}*/ (
    ...
    processAction: function (cm, vim, command) {
      ...
      var actionArgs = /**@type {ActionArgs}*/ (
    ...
    processSearch: function (cm, vim, command) {
      ...
      function pcreLabel() {
        return getOption("pcre")
          ? "(JavaScript regexp: set pcre)"
          : "(Vim regexp: set nopcre)";
      }
      /**
  ...
  var motions = {
    ...
    textObjectManipulation: function (cm, head, motionArgs, vim) {
      ...
      if (mirroredPairs[character]) {
        ...
        while (repeat-- > 0) {
          var repeated = expandWordUnderCursor(
            cm,
            {
              inclusive,
              innerWord: !inclusive,
  ...
  var operators = {
    ...
    indent: function (cm, args, ranges) {
      ...
      if (vim.visualBlock) {
        ...
        var indent = cm.getOption("indentWithTabs")
          ? "\t"
          : " ".repeat(tabSize);
        var cursor;
  ...
  var actions = {
    ...
    joinLines: function (cm, actionArgs, vim) {
      ...
      var curFinalPos = clipCursorToContent(
        cm,
        new Pos(curStart.line, finalCh),
      );
      if (vim.visualMode) {
    ...
    newLineAndEnterInsertMode: function (cm, actionArgs, vim) {
      ...
      if (insertAt.line === cm.firstLine() && !actionArgs.after) {
        ...
        var newlineFn =
          CM.commands.newlineAndIndentContinueComment ||
    ...
    replace: function (cm, actionArgs, vim) {
      ...
      if (replaceWith == "\n") {
        ...
        if (vim.visualBlock) {
          ...
          var replaceWithStrings = replaceWithStr
            .replace(/\t/g, spaces)
            .replace(/[^\n]/g, replaceWith)
            .split("\n");
  ...
  function expandWordUnderCursor(
    cm,
    { inclusive, innerWord, bigWord, noSymbol, multiline },
    cursor,
  ) {
  ...
  function selectCompanionObject(cm, head, symb, inclusive) {
    ...
    var startBracket = cm.scanForBracket(
      new Pos(cur.line, cur.ch + offset),
      -1,
      undefined,
      { bracketRegex: bracketRegexp },
    );
    var endBracket = cm.scanForBracket(
      new Pos(cur.line, cur.ch + offset),
      1,
      undefined,
      { bracketRegex: bracketRegexp },
  ...
  function findBeginningAndEnd(cm, head, symb, inclusive) {
    ...
    // otherwise if the cursor is currently on the closing symbol
      ...
      var stringAfter = /string/.test(
        cm.getTokenTypeAt(offsetCursor(head, 0, 1)),
      );
  ...
  var exCommands = {
      ...
      function parseArgs() {
        if (params.argString) {
          ...
          if (opts[1]) {
            ...
            var decimal =
              opts[1].indexOf("d") != -1 || opts[1].indexOf("n") != -1;
    ...
    delete: function (cm, params) {
      ...
      var lineEnd = isNaN(params.selectionLineEnd)
        ? line
        : params.selectionLineEnd;
      operators.delete(cm, { linewise: true }, [
        { anchor: new Pos(line, 0), head: new Pos(lineEnd + 1, 0) },
      ]);
    },
    /** @arg {CodeMirrorV} cm @arg {ExParams} params*/
  ...
  function doReplace(
    ...
    function replace() {
      var newText = "";
      var match =
        searchCursor.match || (searchCursor.pos && searchCursor.pos.match);
      if (match) {
  ...
  function removeUsedKeys(keys) {
    keys.split(/(<(?:[CSMA]-)*\w+>|.)/i).forEach(function (part) {
      if (usedKeys[part]) usedKeys[part]--;
    });
  }
...
function signal(emitter, type, ...args) {
  var _a;
  var handlers =
    (_a = emitter._handlers) === null || _a === void 0 ? void 0 : _a[type];
  if (!handlers) return;
...
function runHistoryCommand(cm, revert) {
  ...
  let changeStartIndex =
    (_a = cm.curOp) === null || _a === void 0 ? void 0 : _a.$changeStart;
...
class CodeMirror {
  ...
  setBookmark(cursor, options) {
    var assoc = (
      options === null || options === void 0 ? void 0 : options.insertLeft
    )
      ? 1
      : -1;
  ...
  getSearchCursor(query, pos) {
    ...
    return {
      ...
      from: function () {
        return lastCM5Result === null || lastCM5Result === void 0
          ? void 0
          : lastCM5Result.from;
      },
      to: function () {
  ...
  charCoords(pos, mode) {
    ...
    return {
      left:
        ((coords === null || coords === void 0 ? void 0 : coords.left) || 0) -
        rect.left,
      top:
        ((coords === null || coords === void 0 ? void 0 : coords.top) || 0) + d,
      bottom:
        ((coords === null || coords === void 0 ? void 0 : coords.bottom) || 0) +
        d,
    };
  }
  ...
  onChange(update) {
    ...
    if (this.virtualSelection) {
      this.virtualSelection.ranges = this.virtualSelection.ranges.map((range) =>
        range.map(update.changes),
      );
    }
  ...
  getTokenTypeAt(pos) {
    ...
    var type =
      ((_a = node === null || node === void 0 ? void 0 : node.type) === null ||
      _a === void 0
        ? void 0
        : _a.name) || "";
...
CodeMirror.isMac =
  ...
  /*@__PURE__*/ /Mac/.test(navigator.platform);
// --------------------------
CodeMirror.Pos = Pos;
...
CodeMirror.e_stop = function (e) {
  var _a, _b;
  (_a = e === null || e === void 0 ? void 0 : e.stopPropagation) === null ||
  _a === void 0
    ? void 0
    : _a.call(e);
  (_b = e === null || e === void 0 ? void 0 : e.preventDefault) === null ||
  _b === void 0
    ? void 0
    : _b.call(e);
};
...
function findEnclosingTag(cm, pos) {
  ...
  var node =
    (tree === null || tree === void 0 ? void 0 : tree.resolve(offset)) || null;
  while (node) {
    if (
      ((_a = node.firstChild) === null || _a === void 0
        ? void 0
        : _a.type.name) == "OpenTag" &&
      ((_b = node.lastChild) === null || _b === void 0
        ? void 0
        : _b.type.name) == "CloseTag"
    ) {
      return {
...
function hardWrap(cm, options) {
  ...
  while (row <= endRow) {
    ...
    if (line.length > max) {
      ...
      if (space) {
        var indentation =
          (_a = /^\s*/.exec(line)) === null || _a === void 0 ? void 0 : _a[0];
        cm.replaceRange(
          "\n" + indentation,
          new Pos(row, space.start),
          new Pos(row, space.end),
        );
      }
      endRow++;
      ...
      if (nextLine && /\S/.test(nextLine)) {
        ...
        if (
          ...
          cm.replaceRange(
            " ",
            new Pos(row, trimmedLine.length),
            new Pos(row + 1, nextLine.length - trimmedNextLine.length),
          );
          row--;
          endRow--;
...
function configChanged(update) {
  return (
    getDrawSelectionConfig(update.startState) !=
    getDrawSelectionConfig(update.state)
  );
}
...
function measureCursor(cm, view, cursor, primary) {
  ...
  if (fatCursor) {
    ...
    if (node instanceof Text && domAtPos.offset >= node.data.length) {
      if (
        (_a = node.parentElement) === null || _a === void 0
          ? void 0
          : _a.nextSibling
      ) {
        node =
          (_b = node.parentElement) === null || _b === void 0
            ? void 0
            : _b.nextSibling;
    ...
    let charCoords =
      (_d = (_c = view).coordsForChar) === null || _d === void 0
        ? void 0
        : _d.call(_c, head);
    if (charCoords) {
    ...
    if (!letter || letter == "\n" || letter == "\r") {
      ...
      if (nextPos) {
        left =
          nextPos.left - (nextPos.left - pos.left) / parseInt(style.tabSize);
      }
    } else if (
      /[\uD800-\uDBFF]/.test(letter) &&
...
const vimPlugin = /*@__PURE__*/ ViewPlugin.fromClass(
  class {
    constructor(view) {
      this.status = "";
      this.query = null;
        ...
        for (let tr of update.transactions)
          for (let effect of tr.effects) {
            if (effect.is(setSearchQuery)) {
              let forVim =
                (_a = effect.value) === null || _a === void 0
                  ? void 0
                  : _a.forVim;
              if (!forVim) {
                this.highlight(null);
      ...
      return [
        EditorView.inputHandler.of((view, from, to, text) => {
          ...
          if (
            vim &&
            !vim.insertMode &&
            !((_b = cm.curOp) === null || _b === void 0 ? void 0 : _b.isVimOp)
          ) {
            if (text === "\0\0") {
              return true;
            }
            ...
            if (text.length == 1 && vimPlugin.useNextTextInput) {
              ...
              if (vimPlugin.compositionText) {
                ...
                var textInDoc = view.state.sliceDoc(
                  head - toRemove.length,
                  head,
                );
                if (toRemove === textInDoc) {
                  ...
                  cm.replaceRange(
                    "",
                    cm.posFromIndex(head - toRemove.length),
                    pos,
                  );
                }
              }
