<script lang="ts">
  ...
  const { sectionState: rightState } = API.section.useSectionStore<"vertical">({
    id: LAYOUT.RIGHT_SIDEBAR,
  });
</script>

<button
  onclick={() => toggleLayout(LAYOUT.RIGHT_SIDEBAR)}
  class={["rounded-sm shrink-0 hover:bg-zinc-900", className]}
>
