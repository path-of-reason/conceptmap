...
import type { Tab, GridCell, WorkspaceLayout } from "./types.d.ts";
...
export const useLayoutStore = () => {
  ...
  function addTab(tab: Tab) {
    return layoutStore.tabs.push(tab);
  }
  function removeTab(tabId: string) {
    return () =>
      (layoutStore.tabs = layoutStore.tabs.filter((tab) => tab.id !== tabId));
  }
  function getTabById(tabId: string): Tab | undefined {
    return layoutStore.tabs.find((tab) => tab.id === tabId);
  }
  // cell actions
  function assignTabToCell(tabId: string | null, cellId: string) {
    const cell = layoutStore.root.cells.find((c) => c.id === cellId);
    if (cell) {
  ...
  function createNewTab() {
    ...
    addTab({
      id: newTabId,
      title: `New Tab ${layoutStore.tabs.length + 1}`,
      type: "file",
      content: `Content for new tab ${layoutStore.tabs.length + 1}`,
    });
    // 새로 생성된 탭을 현재 포커스된 셀에 자동으로 할당합니다.
    if (layoutStore.root.focusedCellId) {
      assignTabToCell(newTabId, layoutStore.root.focusedCellId);
    } else if (layoutStore.root.cells.length > 0) {
      // 포커스된 셀이 없으면 첫 번째 셀에 할당
      assignTabToCell(newTabId, layoutStore.root.cells[0].id);
      focusCell(layoutStore.root.cells[0].id); // 첫 번째 셀에 포커스
    }
  }
...
export const CellUtil = {
  splitVertical: () => {
    const focusCellId = layoutStore.root.focusedCellId;
  ...
  decreaseCellWidth: () => {
    const focusCellId = layoutStore.root.focusedCellId;
    if (focusCellId) {
      const focusedCell = layoutStore.root.cells.find(
        (c) => c.id === focusCellId,
      );
      if (
        focusedCell &&
        focusedCell.col !== undefined // 이제 마지막 열도 조작 가능하므로 이 조건은 필요없음: && layoutStore.root.colSizes.length > focusedCell.col + focusedCell.colspan
      ) {
  ...
  decreaseCellHeight: () => {
    ...
    if (focusCellId) {
      ...
      if (
        ...
        layoutStore.root.rowSizes.length > focusedCell.row + focusedCell.rowspan
      ) {
        const currentFrValue = parseFloat(
          layoutStore.root.rowSizes[focusedCell.row].toString(),
        );
        const delta = -0.1;
...
function resizeCellHeight(rowIndex: number, newFrValue: number) {
  const currentFrValues = layoutStore.root.rowSizes.map((s) =>
  ...
  layoutStore.root.rowSizes[rowIndex] = `${newFr1}fr`;
  layoutStore.root.rowSizes[rowIndex + 1] = `${newFr2}fr`;

  // $state는 배열 요소 변경을 감지하지만, 배열 자체를 교체하면 더 확실하게 Svelte의 반응성을 트리거할 수 있습니다.
  layoutStore.root.rowSizes = [...layoutStore.root.rowSizes];
  console.log(
...
function resizeCellWidth(colIndex: number, newFrValue: number) {
  const currentFrValues = layoutStore.root.colSizes.map((s) =>
    parseFloat(s.toString()),
  );
  ...
  layoutStore.root.colSizes[colIndex] = `${newFr1}fr`;
  layoutStore.root.colSizes[colIndex + 1] = `${newFr2}fr`;

  // Svelte 반응성 트리거
  layoutStore.root.colSizes = [...layoutStore.root.colSizes];
}
function resizeGridCol(colIndex: number, deltaFr: number) {
  ...
  if (remainingFrCount > 0) {
    ...
    for (let i = 0; i < currentFrValues.length; i++) {
      if (i === colIndex) {
        layoutStore.root.colSizes[i] = `${newFrForTarget}fr`;
      } else {
        ...
        layoutStore.root.colSizes[i] = `${adjustedFr}fr`;
      }
    }
  } else {
    // 열이 하나뿐인 경우
    layoutStore.root.colSizes[colIndex] = `${newFrForTarget}fr`;
  }
...
function focusCell(cellId: string | null) {
  return (layoutStore.root.focusedCellId = cellId);
}
/**
...
function addPanel(
  ...
  const targetCell = layoutStore.root.cells.find((c) => c.id === targetCellId);
  if (!targetCell) {
  ...
  switch (direction) {
    case "right":
      layoutStore.root.cols++;
      layoutStore.root.colSizes.splice(
        targetCell.col + targetCell.colspan,
        0,
        "1fr",
      );
      layoutStore.root.cells.forEach((cell) => {
        if (cell.col >= targetCell.col + 1) cell.col++;
      });
      for (let i = 0; i < layoutStore.root.rows; i++) {
    ...
    case "below":
      // if (layoutStore.root.rows > 8) break;
      layoutStore.root.rows++;
      layoutStore.root.rowSizes.splice(
      ...
      for (let i = 0; i < layoutStore.root.cols; i++) {
  ...
  layoutStore.root.cells = [...layoutStore.root.cells, ...newCells];

  // GridCells 배열 정렬 (그리드 렌더링 순서에 영향)
  layoutStore.root.cells.sort((a, b) => a.row - b.row || a.col - b.col);
...
function removePanel(cellId: string, direction: "vertical" | "horizontal") {
  const targetCell = layoutStore.root.cells.find((c) => c.id === cellId);
  if (!targetCell) return;

  if (direction === "vertical" && layoutStore.root.cols > 1) {
    const currentCol = targetCell.col + targetCell.colspan;
    moveFocus("left");
    layoutStore.root.cols--;
    layoutStore.root.colSizes = layoutStore.root.colSizes.filter(
      (_, i) => i !== targetCell.col,
    );
    layoutStore.root.cells = layoutStore.root.cells.filter(
  ...
  if (direction === "horizontal" && layoutStore.root.rows > 1) {
    const currentRow = targetCell.row + targetCell.rowspan;
    moveFocus("up");
    layoutStore.root.rows--;
    layoutStore.root.rowSizes = layoutStore.root.rowSizes.filter(
...
function moveFocus(direction: "up" | "down" | "left" | "right") {
  const cells = layoutStore.root.cells;
  const currentFocusCellId = layoutStore.root.focusedCellId;
  const currentFocusCell = cells.find(
    (cell) => cell.id === currentFocusCellId,
  )!;
  if (!currentFocusCell) {
    layoutStore.root.focusedCellId = cells[0].id;
    return;
  }
  ...
  switch (direction) {
    case "right":
      if (currentFocusCell.col === layoutStore.root.cols - 1) nextCol = 0;
      else nextCol = currentFocusCell.col + 1;
      nextRow = currentFocusCell.row;
      break;
    case "left":
      if (currentFocusCell.col === 0) nextCol = layoutStore.root.cols - 1;
      else nextCol = currentFocusCell.col - 1;
      nextRow = currentFocusCell.row;
      break;
    case "down":
      if (currentFocusCell.row === layoutStore.root.rows - 1) nextRow = 0;
      else nextRow = currentFocusCell.row + 1;
      nextCol = currentFocusCell.col;
      break;
    case "up":
      if (currentFocusCell.row === 0) nextRow = layoutStore.root.rows - 1;
      else nextRow = currentFocusCell.row - 1;
      nextCol = currentFocusCell.col;
      break;
  }
  const nextCell = layoutStore.root.cells.find(
